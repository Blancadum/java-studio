...
export async function updateUserApiConfig(email: string, config: Partial<UserApiConfig>): Promise<UserProfile> {
  ...
  if (config.geminiUserKey !== undefined) {
    profile.apiConfig.geminiUserKey = config.geminiUserKey;
    profile.apiConfig.maskedGeminiKey = maskKey(config.geminiUserKey);
  }
  if (config.backupUserKey !== undefined) {
    profile.apiConfig.backupUserKey = config.backupUserKey;
    profile.apiConfig.maskedBackupKey = maskKey(config.backupUserKey);
  }
...
export async function saveUserSession(email: string, sessionData: Omit<SavedSession, 'id' | 'createdAt'>): Promise<SavedSession> {
  ...
  profile.totalTokensSaved += (newSession.tokensSaved || 15000);
  await kv.set(userKey(email), profile);
  return newSession;
}
