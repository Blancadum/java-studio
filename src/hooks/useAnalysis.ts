...
export function useAnalysis(userProfile: UserProfile | null, onOpenAuthModal: () => void, onUpdateUserProfile: (user: UserProfile) => void) {
...
public class ReservaNotFoundException extends Exception {
  ...
  }, [setNoFiles, setFixedFiles, setTeacherDoc, setActiveMode, setAnalysisResult, setProposedFiles]);
  ...
  }, [setActiveMode, setAnalysisResult, setArchitectureGuide, setPreSubmissionAudit, setSonarQuality]);
  ...
  const handleReset = useCallback(() => {
    ...
    setTeacherDoc('');
    setProposedFiles([]);
  }, [setAnalysisResult, setArchitectureGuide, setPreSubmissionAudit, setSonarQuality, setNoFiles, setFixedFiles, setTeacherDoc, setProposedFiles]);

  // Memoize API config to avoid re-creating on every render
  const ANALYSIS_API_CONFIG = useMemo(
    ...
    [setAnalysisResult, setArchitectureGuide, setPreSubmissionAudit, setSonarQuality, setProposedFiles, setFixedFiles]
  );
  ...
  const handleStartAnalysis = useCallback(async (
    ...
    try {
      ...
      const payload = config.buildPayload({ inputNo, inputFixed, inputTeacher, modeSpecificOptions, apiOptions });
  ...
  const handleApplyProposal = useCallback(async (proposal: ImprovementProposal) => {
    ...
    if (existingIndex >= 0) {
      let finalCode = proposal.proposedCode; // Default to the proposal's code
      try {
  ...
  const handleSaveCurrentSession = useCallback(async () => {
    ...
    if (activeMode === 'FEEDBACK_REVISION' && analysisResult) {
      ...
      tokensSaved = JSON.stringify(analysisResult).length * 2; // Estimación simple
      ...
      tokensSaved = JSON.stringify(preSubmissionAudit).length; // Estimación simple
      ...
      tokensSaved = JSON.stringify(sonarQuality).length; // Estimación simple
    }
